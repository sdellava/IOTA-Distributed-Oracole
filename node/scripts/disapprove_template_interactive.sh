...
Usage:
  ./scripts/disapprove_template_interactive.sh [--network <devnet|testnet>] [--node <id>] [--signer <addr_or_alias>] [--env-file ./.env]
EOF
}
...
NETWORK_RAW="${NETWORK_OVERRIDE:-${IOTA_NETWORK:-}}"
NETWORK_RAW="$(echo "$NETWORK_RAW" | tr '[:upper:]' '[:lower:]' | xargs)"
case "$NETWORK_RAW" in
  dev|local|localnet) NETWORK_RAW="devnet" ;;
  test) NETWORK_RAW="testnet" ;;
  main) NETWORK_RAW="mainnet" ;;
esac
if [[ -z "$NETWORK_RAW" ]]; then
  NETWORK_RAW="devnet"
fi
case "$NETWORK_RAW" in
  devnet|testnet) ;;
  *)
    echo "[error] unsupported network: ${NETWORK_RAW}. Use --network devnet or --network testnet." >&2
    exit 1
    ;;
esac
NET_PREFIX="$(echo "$NETWORK_RAW" | tr '[:lower:]' '[:upper:]')"
MOVE_SRC="${PROJECT_DIR}/../move/${NETWORK_RAW}/oracle_system_state_${NETWORK_RAW}/sources/iota_oracle_system_state.move"
