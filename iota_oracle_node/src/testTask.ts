...
function defaultCandidatePaths(input: string): string[] {
  ...
  return [
    ...
    path.resolve(cwd, "..", "iota_oracle_client", trimmed),
    path.resolve(cwd, "..", "iota_oracle_client", "examples", trimmed),
    path.resolve(cwd, "..", "iota_oracle_client", "examples", baseName),
  ];
}
...
function loadTaskJson(taskArg?: string): { sourceLabel: string; taskObj: any } {
  if (!taskArg) {
    const defaults = [
      ...
      "../iota_oracle_client/examples/task_weather.json",
    ];
