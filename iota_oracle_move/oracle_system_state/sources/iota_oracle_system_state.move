module iota_oracle_system_state::systemState {
    ...
    const EProposalAlreadyActive: u64 = 60;
    const ENoActiveProposal: u64 = 61;
    ...
    public struct State has key, store {
        ...
        // active template proposal
        template_proposal_id: u64,
        template_proposal_active: u8,
        template_proposal_kind: u8,
        template_proposal_deadline_ms: u64,
        template_proposal_approvals: u64,
        template_proposal_electorate_size: u64,

        proposed_template_id: u64,
        proposed_task_type: vector<u8>,
        proposed_is_enabled: u8,
        proposed_base_price_iota: u64,
        proposed_max_input_bytes: u64,
        proposed_max_output_bytes: u64,
        proposed_included_download_bytes: u64,
        proposed_price_per_download_byte_iota: u64,
        proposed_allow_storage: u8,
        proposed_min_retention_days: u64,
        proposed_max_retention_days: u64,
        proposed_price_per_retention_day_iota: u64,
    }
    ...
    fun init(ctx: &mut TxContext) {
        ...
        let st = State {
            ...
            template_proposal_active: 0,
            template_proposal_kind: PROPOSAL_KIND_NONE,
            template_proposal_deadline_ms: 0,
            template_proposal_approvals: 0,
            template_proposal_electorate_size: 0,

            proposed_template_id: 0,
            proposed_task_type: vector::empty(),
            proposed_is_enabled: 0,
            proposed_base_price_iota: 0,
            proposed_max_input_bytes: 0,
            proposed_max_output_bytes: 0,
            proposed_included_download_bytes: 0,
            proposed_price_per_download_byte_iota: 0,
            proposed_allow_storage: 0,
            proposed_min_retention_days: 0,
            proposed_max_retention_days: 0,
            proposed_price_per_retention_day_iota: 0,
        };
    ...
    public entry fun propose_task_template_upsert(
        ...
        maybe_expire_template_proposal(st, clock);
        assert!(st.template_proposal_active == 0, EProposalAlreadyActive);
        ...
        st.template_proposal_active = 1;
        st.template_proposal_kind = PROPOSAL_KIND_TEMPLATE_UPSERT;
        st.template_proposal_deadline_ms = now + proposal_timeout_ms;
        st.template_proposal_approvals = 0;
        st.template_proposal_electorate_size = electorate;

        st.proposed_template_id = template_id;
        st.proposed_task_type = task_type;
        st.proposed_is_enabled = is_enabled;
        st.proposed_base_price_iota = base_price_iota;
        st.proposed_max_input_bytes = max_input_bytes;
        st.proposed_max_output_bytes = max_output_bytes;
        st.proposed_included_download_bytes = included_download_bytes;
        st.proposed_price_per_download_byte_iota = price_per_download_byte_iota;
        st.proposed_allow_storage = allow_storage;
        st.proposed_min_retention_days = min_retention_days;
        st.proposed_max_retention_days = max_retention_days;
        st.proposed_price_per_retention_day_iota = price_per_retention_day_iota;
        ...
        event::emit(TaskTemplateProposalCreated {
            ...
            template_id,
            proposer,
            deadline_ms: st.template_proposal_deadline_ms,
    ...
    public entry fun propose_task_template_remove(
        ...
        st.template_proposal_id = pid;
        st.template_proposal_active = 1;
        st.template_proposal_kind = PROPOSAL_KIND_TEMPLATE_REMOVE;
        st.template_proposal_deadline_ms = now + proposal_timeout_ms;
        st.template_proposal_approvals = 0;
        st.template_proposal_electorate_size = electorate;

        st.proposed_template_id = template_id;
        st.proposed_task_type = vector::empty();
        st.proposed_is_enabled = 0;
        st.proposed_base_price_iota = 0;
        st.proposed_max_input_bytes = 0;
        st.proposed_max_output_bytes = 0;
        st.proposed_included_download_bytes = 0;
        st.proposed_price_per_download_byte_iota = 0;
        st.proposed_allow_storage = 0;
        st.proposed_min_retention_days = 0;
        st.proposed_max_retention_days = 0;
        st.proposed_price_per_retention_day_iota = 0;
    ...
    public entry fun approve_task_template_proposal(
        ...
        assert!(st.template_proposal_active == 1, ENoActiveProposal);
        assert!(
            st.template_proposal_kind == PROPOSAL_KIND_TEMPLATE_UPSERT ||
            st.template_proposal_kind == PROPOSAL_KIND_TEMPLATE_REMOVE,
        ...
        let now = timestamp_ms(clock);
        assert!(now <= st.template_proposal_deadline_ms, EProposalExpired);

        let sender = iota::tx_context::sender(ctx);
        assert!(has_node_ref(st, sender), ENotFound);

        let key = TemplateProposalApprovalKey {
            proposal_id: st.template_proposal_id,
            voter: sender,
        };
        assert!(
        ...
        st.template_proposal_approvals = st.template_proposal_approvals + 1;

        let needed = majority_threshold(st.template_proposal_electorate_size);

        event::emit(TaskTemplateProposalApproved {
            proposal_id: st.template_proposal_id,
            proposal_kind: st.template_proposal_kind,
            template_id: st.proposed_template_id,
            by: sender,
            approvals: st.template_proposal_approvals,
            approvals_needed: needed,
        });

        if (st.template_proposal_approvals >= needed) {
            if (st.template_proposal_kind == PROPOSAL_KIND_TEMPLATE_UPSERT) {
                apply_template_upsert(st);
                event::emit(TaskTemplateUpserted {
                    template_id: st.proposed_template_id,
                    is_enabled: st.proposed_is_enabled,
                });
            } else {
                apply_template_remove(st);
    ...
    */
    ...
    public fun template_proposal_active(st: &State): u8 { st.template_proposal_active }
    public fun template_proposal_id(st: &State): u64 { st.template_proposal_id }
    public fun template_proposal_kind(st: &State): u8 { st.template_proposal_kind }
    public fun template_proposal_deadline_ms(st: &State): u64 { st.template_proposal_deadline_ms }
    public fun template_proposal_approvals(st: &State): u64 { st.template_proposal_approvals }
    public fun template_proposal_electorate_size(st: &State): u64 { st.template_proposal_electorate_size }
    public fun proposed_template_id(st: &State): u64 { st.proposed_template_id }
    ...
    fun maybe_expire_template_proposal(st: &mut State, clock: &Clock) {
        if (st.template_proposal_active == 0) return;
        let now = timestamp_ms(clock);
        if (now > st.template_proposal_deadline_ms) {
            let pid = st.template_proposal_id;
            let kind = st.template_proposal_kind;
            let template_id = st.proposed_template_id;
            clear_template_proposal(st);
    ...
    fun apply_template_upsert(st: &mut State) {
        let key = TaskTemplateKey { template_id: st.proposed_template_id };
        let tpl = TaskTemplate {
            template_id: st.proposed_template_id,
            task_type: copy_bytes(&st.proposed_task_type),
            is_enabled: st.proposed_is_enabled,
            base_price_iota: st.proposed_base_price_iota,
            max_input_bytes: st.proposed_max_input_bytes,
            max_output_bytes: st.proposed_max_output_bytes,
            included_download_bytes: st.proposed_included_download_bytes,
            price_per_download_byte_iota: st.proposed_price_per_download_byte_iota,
            allow_storage: st.proposed_allow_storage,
            min_retention_days: st.proposed_min_retention_days,
            max_retention_days: st.proposed_max_retention_days,
            price_per_retention_day_iota: st.proposed_price_per_retention_day_iota,
        };
    ...
    fun apply_template_remove(st: &mut State) {
        let key = TaskTemplateKey { template_id: st.proposed_template_id };
        assert!(dynamic_field::exists_<TaskTemplateKey>(&st.id, key), ETemplateNotFound);
        let _old: TaskTemplate =
            dynamic_field::remove<TaskTemplateKey, TaskTemplate>(&mut st.id, key);
    ...
    fun clear_template_proposal(st: &mut State) {
        st.template_proposal_active = 0;
        st.template_proposal_kind = PROPOSAL_KIND_NONE;
        st.template_proposal_deadline_ms = 0;
        st.template_proposal_approvals = 0;
        st.template_proposal_electorate_size = 0;

        st.proposed_template_id = 0;
        st.proposed_task_type = vector::empty();
        st.proposed_is_enabled = 0;
        st.proposed_base_price_iota = 0;
        st.proposed_max_input_bytes = 0;
