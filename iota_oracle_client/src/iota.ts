    ...
    get(_target, prop) {
      ...
      return (...args: unknown[]) => {
        if (!shouldRetryMethod(prop) || urls.length <= 1) {
          const direct = Reflect.get(activeClient as object, prop) as (...params: unknown[]) => unknown;
...
export function iotaClient(): IotaClient {
  ...
  const fallbackUrl = getFullnodeUrl(network === 'localnet' ? 'devnet' : network);

  const urls = [...new Set([
    ...parseRpcUrls(process.env.IOTA_RPC_URLS?.trim()),
    ...parseRpcUrls(process.env.IOTA_RPC_URL?.trim()),
    fallbackUrl,
  ])];
